...
function determineKeepsakeNextAction(row) {
  const status = row[6] || '';
  const formCompleteRaw = row[41];
  // Handle both boolean and string values from sheets
  const formComplete = formCompleteRaw === true || 
                       String(formCompleteRaw).toLowerCase() === 'true';
  ...
  console.log(`DEBUG: Status=${status}, FormComplete=${formComplete}, Raw=${formCompleteRaw}`);
